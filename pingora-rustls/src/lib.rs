...
/// When `ring` is disabled (e.g. via `rustls-no-provider`), this is a no-op and the
...
///
/// When the `ring` feature is enabled the hash is computed directly via
/// `ring::digest`.  Otherwise the SHA-256 implementation is obtained from
/// the installed [`CryptoProvider`].
pub fn hash_certificate(cert: &CertificateDer) -> Vec<u8> {
    ...
    #[cfg(not(feature = "ring"))]
    {
        ...
        let provider = CryptoProvider::get_default().expect(
            "no CryptoProvider installed — call CryptoProvider::install_default() before use",
        );
        let hash = provider
            .cipher_suites
            .iter()
